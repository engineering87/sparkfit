...
def write_config(updates: dict, path: "str | None" = None) -> str:
    """Merge friendly-key updates into the config file; return the path written."""
    ...
    lines = ["# sparkfit config (generated by `sparkfit doctor`)"]
...
def cmd_cluster(args: argparse.Namespace) -> None:
    ...
    mult = 1 + (n - 1) * args.scaling      # 1 node -> 1.0; each extra node adds `scaling`
    ...
    print(dim(f"  fabric: {fabric_gbps:.0f} Gb/s (~{fabric_gb_s:.0f} GB/s) between nodes, over 10x "
              f"slower than {SPARK['bandwidth_gbps']:.0f} GB/s on-node."))
    if args.parallelism == "tp":
...
def cmd_doctor(args: argparse.Namespace) -> None:
    ...
    if args.measured_tok_s is not None:
        ...
        saved = write_config({"efficiency": round(eff, 3)}) if args.save else None
        if args.json:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    dr.add_argument("--bandwidth", type=float, default=None)
